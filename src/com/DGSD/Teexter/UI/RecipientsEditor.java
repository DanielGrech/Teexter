...
public class RecipientsEditor extends MultiAutoCompleteTextView {
    ...
    private boolean isValidAddress(String number) {
    	return PhoneNumberUtils.isWellFormedSmsAddress(number) || number.matches(Patterns.EMAIL_ADDRESS.pattern());
    }

    public boolean hasValidRecipient(boolean isMms) {
        for (String number : mTokenizer.getNumbers()) {
            if (isValidAddress(number))
                return true;
        }
        return false;
    }
    ...
    public boolean containsEmail() {
        ...
        for (String number : numbers) {
            if (number.matches(Patterns.EMAIL_ADDRESS.pattern()));
                return true;
        }
        return false;
    }
    ...
    public static CharSequence contactToToken(Contact c) {
        SpannableString s = new SpannableString(ContactUtils.formatNameAndNumber(c.name, c.phone));
        int len = s.length();
        ...
        s.setSpan(new Annotation("number", c.phone), 0, len,
        ...
        if ((mLongPressedPosition >= 0)) {
            ...
            if (mLongPressedPosition <= text.length()) {
                ...
                if (end != start) {
                    ...
                    Contact c = ContactUtils.getContactFromPhone(getContext(), number);
